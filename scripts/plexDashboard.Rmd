...
renderPlot({
  if(rv$chooseYear >= 2006) {
    ## Match plex tracks with lastfm plays, make everything lower case and remove punctuation
    plexCount <- rv$plexYear %>% 
      unite(col = 'match', albumArtist, album, sep = " - ", remove = FALSE) %>% 
      mutate(match = getSlugs(match)) %>% 
      distinct(albumArtist, album, match, albumKey)
    lastfmCount <- rv$lastfm %>% 
      filter(year(date) == input$chooseYear) %>%
    ...
    albums <- right_join(
      ...
      arrange(desc(n)) %>% 
      mutate(inPlex = ifelse(is.na(albumKey), 1, 0)) %>% 
      mutate(inPlex = forcats::as_factor(inPlex),
             rank = row_number())
    
    } else {
      plexYear <- rv$plexYear %>% 
    ...
    tracks <- left_join(
      ...
      arrange(desc(n)) %>% 
      mutate(inPlex = ifelse(is.na(key), 1, 0)) %>% 
      mutate(inPlex = forcats::as_factor(inPlex),
             rank = row_number())
