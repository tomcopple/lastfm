...
getPlex <- function(refresh = FALSE) {
  ...
  if (refresh) {
      ...
      plexToken <- "YTbYV3s5vkVVco6stFDW"
    plexRaw <- content(GET("http://192.168.1.99:32400/library/sections/3/search?type=10", add_headers("X-Plex-Token" = plexToken)), type = 'text') %>% 
      jsonlite::fromJSON() %>% 
    ...
    rdrop2::drop_upload(file = here::here('tempData', "plexDB.csv"), 
      ...
      rdrop2::drop_download("R/lastfm/plexDB.csv",
                          local_path = "plexDB.csv", dtoken = token,
                          overwrite = T)

    plex <- readr::read_csv("plexDB.csv", lazy = F) %>%
       as_tibble()
...
getLastfm <- function(refresh = TRUE) {
    ...
    rdrop2::drop_download("R/lastfm/tracks.csv",
                          local_path = "tracks.csv", dtoken = token,
                          overwrite = T)
    ...
    if (refresh) {
        ...
        rdrop2::drop_upload(file = "tracks.csv", path = "R/lastfm", dtoken = token)
    }
