...
refresh_dropbox_access_token <- function(token_obj, app_key, app_secret) {
  if (is.null(token_obj)) {
    stop("Dropbox token object is missing")
  }

  refresh_token <- NULL
  if (!is.null(token_obj$refresh_token) && nzchar(token_obj$refresh_token)) {
    refresh_token <- token_obj$refresh_token
  } else if (!is.null(token_obj$credentials$refresh_token) && nzchar(token_obj$credentials$refresh_token)) {
    refresh_token <- token_obj$credentials$refresh_token
  }

  if (is.null(refresh_token)) {
    if (!is.null(token_obj$credentials$access_token) && nzchar(token_obj$credentials$access_token)) {
      return(token_obj$credentials$access_token)
    }
    if (!is.null(token_obj$access_token) && nzchar(token_obj$access_token)) {
      return(token_obj$access_token)
    }
    stop("No usable Dropbox token found in dropbox.rds")
  }

  if (!nzchar(app_key) || !nzchar(app_secret)) {
    stop("DROPBOX_KEY and DROPBOX_SECRET must be set to refresh Dropbox token")
  }

  refresh_resp <- httr2::request("https://api.dropboxapi.com/oauth2/token") %>%
    httr2::req_auth_basic(username = app_key, password = app_secret) %>%
    httr2::req_body_form(
...
getLastfmShiny <- function(refresh = TRUE) {
    ...
    lastfmDownload <- dropboxr::download_dropbox_file("/R/lastfm/tracks.csv", token = access_token)
    ...
    if (refresh) {
        ...
        dropboxr::upload_df_to_dropbox(lastfmUpload, "/R/lastfm/tracks.csv", token = access_token)
