...
slug <- "http://192.168.1.99:32400/playlists/29599/items"

## Download playlist
play18raw <- content(httr::GET(slug, add_headers("X-Plex-Token" = token)), type = 'text') %>% 
    jsonlite::fromJSON() %>% 
    magrittr::extract2('MediaContainer') %>% 
    magrittr::extract2('Metadata')
play18 <- play18raw %>% select(
...
plex <- getPlex(T) %>% 
  ...
  mutate(lowerAlbum = str_remove_all(str_to_lower(lowerAlbum), "[[:punct:]]"),
         lowerTrack = str_remove_all(str_to_lower(lowerTrack), "[[:punct:]]"))
tracks <- getLastfm(T) %>% filter(year(date) == 2018)
