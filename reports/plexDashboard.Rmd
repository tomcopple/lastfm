...
<style>
/* Force full-height behavior for Missing from Discogs (AI) table */
#missing-discogs-ai-table-wrap,
#missing-plex-discogs-table-wrap,
#missing-plex-lastfm-table-wrap,
#missing-discogs-ai-table-wrap .html-widget,
#missing-plex-discogs-table-wrap .html-widget,
#missing-plex-lastfm-table-wrap .html-widget,
#missing-discogs-ai-table-wrap .datatables,
#missing-plex-discogs-table-wrap .datatables,
#missing-plex-lastfm-table-wrap .datatables,
#missing-discogs-ai-table-wrap .dataTables_wrapper,
#missing-plex-discogs-table-wrap .dataTables_wrapper,
#missing-plex-lastfm-table-wrap .dataTables_wrapper,
#missing-discogs-ai-table-wrap .dataTables_scroll {
#missing-plex-discogs-table-wrap .dataTables_scroll,
#missing-plex-lastfm-table-wrap .dataTables_scroll {
...
#missing-discogs-ai-table-wrap .dataTables_scrollBody,
#missing-plex-discogs-table-wrap .dataTables_scrollBody,
#missing-plex-lastfm-table-wrap .dataTables_scrollBody {
  height: calc(100vh - 290px) !important;
  max-height: calc(100vh - 290px) !important;
}

/* Override flexdashboard row inline height limit for this section */
#missing-discogs-ai-row {
  height: calc(100vh - 115px) !important;
  min-height: calc(100vh - 115px) !important;
  flex: 1 1 auto !important;
}

div.dashboard-row:has(#missing-discogs-ai-table-wrap) {
  height: calc(100vh - 115px) !important;
  min-height: calc(100vh - 115px) !important;
  flex: 1 1 auto !important;
}

div.dashboard-row:has(#missing-plex-discogs-table-wrap),
div.dashboard-row:has(#missing-plex-lastfm-table-wrap) {
  height: calc(100vh - 115px) !important;
  min-height: calc(100vh - 115px) !important;
  flex: 1 1 auto !important;
}
...
document.addEventListener('DOMContentLoaded', function () {
  function forceRowHeightFromWrapper(wrapperId) {
    var wrap = document.getElementById(wrapperId);
    if (!wrap) return;
    var row = wrap.closest('.dashboard-row');
    if (!row) return;
    row.style.setProperty('height', 'calc(100vh - 115px)', 'important');
    row.style.setProperty('min-height', 'calc(100vh - 115px)', 'important');
    row.style.setProperty('flex', '1 1 auto', 'important');
  }

  function forceDashboardRowsHeight() {
    forceRowHeightFromWrapper('missing-discogs-ai-table-wrap');
    forceRowHeightFromWrapper('missing-plex-discogs-table-wrap');
    forceRowHeightFromWrapper('missing-plex-lastfm-table-wrap');
  }

  forceDashboardRowsHeight();
  window.addEventListener('resize', forceDashboardRowsHeight);
  setTimeout(forceDashboardRowsHeight, 300);
  setTimeout(forceDashboardRowsHeight, 1000);
});
</script>
...
load_missing_discogs_ai <- function(path = NULL) {
  if (is.null(path)) {
    path <- here::here("data", "exports", "missingDiscogsAISuggestions.csv")
  }

  empty_data <- tibble::tibble(
    rank = integer(),
    recommendation_source = character(),
    artist = character(),
    album = character(),
    ai_why = character(),
    ...
    owned_plex_albums_by_artist = integer(),
    avg_plex_rating_for_artist = numeric(),
    baseline_score = numeric(),
    baseline_reason = character(),
    join_key = character()
  )
  ...
  tryCatch(
    ...
    error = function(e) {
      warning(paste0("Could not load Discogs AI suggestions file '", path, "': ", e$message))
      empty_data
    }
  )
}
...
safe_getPlex <- function(refresh = FALSE) {
  tryCatch(
    {
      data <- getPlex(refresh)
      if (is.null(data) || !is.data.frame(data)) {
        warning("getPlex() returned non-tabular data; using empty Plex dataset")
        return(empty_plex())
      }
      tibble::as_tibble(data)
    },
    error = function(e) {
      warning(paste0("Could not load Plex from Dropbox/API: ", e$message))

      fallback_path <- here::here("data", "exports", "plexDB.csv")
      if (file.exists(fallback_path)) {
        fallback <- tryCatch(
          readr::read_csv(fallback_path, show_col_types = FALSE),
          error = function(read_err) {
            warning(paste0("Could not load local fallback plexDB.csv: ", read_err$message))
            NULL
          }
        )

        if (!is.null(fallback) && is.data.frame(fallback)) {
          message("Using local fallback Plex data from data/exports/plexDB.csv")
          return(tibble::as_tibble(fallback))
        }
      }
...
missingDiscogsAIRefreshTick <- shiny::reactiveVal(0)

missingDiscogsAICombined <- reactive({
  missingDiscogsAIRefreshTick()
  load_missing_discogs_ai()
})

observeEvent(eventExpr = input$refreshPlex,
             handlerExpr = {
               showNotification("Updating Plex...")
               rv$plex <- safe_getPlex(T)
...
observeEvent(eventExpr = input$refreshMissingDiscogsAI,
             handlerExpr = {
               note_id <- shiny::showNotification("Refreshing Missing from Discogs (AI) suggestions...", duration = NULL)

               script_path <- here::here("R", "analysis", "2026-02-missingMajorWorks.R")
               rscript_bin <- file.path(R.home("bin"), "Rscript")

               run_result <- tryCatch(
                 {
                   system2(rscript_bin, script_path, stdout = TRUE, stderr = TRUE)
                 },
                 error = function(e) e
               )

               shiny::removeNotification(note_id)

               if (inherits(run_result, "error")) {
                 shiny::showNotification(
                   paste0("Failed to refresh AI suggestions: ", run_result$message),
                   type = "error",
                   duration = 10
                 )
               ...
               if (!is.null(run_status) && !identical(run_status, 0L)) {
                 error_text <- if (length(run_result) > 0) {
                   stringr::str_c(tail(run_result, 8), collapse = "\n")
                 } else {
                   paste0("Rscript exited with status ", run_status)
                 }

                 shiny::showNotification(
                   paste0("Failed to refresh AI suggestions. ", error_text),
                   type = "error",
                   duration = NULL
                 )
                 return(invisible(NULL))
               ...
               missingDiscogsAIRefreshTick(missingDiscogsAIRefreshTick() + 1)

               source_used <- "heuristic fallback"
               if (length(run_result) > 0 && any(stringr::str_detect(run_result, "Source: openai"))) {
                 source_used <- "OpenAI"
               }

               reason_line <- run_result[stringr::str_detect(run_result, "^Reason:")]
               reason_text <- if (length(reason_line) > 0) {
                 stringr::str_trim(stringr::str_remove(reason_line[[length(reason_line)]], "^Reason:"))
               } else {
                 "No reason returned"
               }

               shiny::showNotification(
                 paste0("Missing Discogs suggestions refreshed (", source_used, "). Reason: ", reason_text),
                 type = "message"
               )
             })
```
...
```{r}
initial_artist_choices <- artist_choices(plex)

shiny::radioButtons(
  inputId = "compareArtist",
  label = "Select artist",
  choices = initial_artist_choices,
  # options = list(placeholder = "Choose an artist"),
  selected = if (length(initial_artist_choices) > 0) initial_artist_choices[[1]] else character(0)
)
```
...
```{r}
year_values <- if (nrow(plex) > 0) {
  max_year <- suppressWarnings(max(plex %>% dplyr::filter(!is.na(year)) %>% dplyr::pull(year), na.rm = TRUE))
  if (is.finite(max_year) && max_year >= 2000) {
    as.character(2000:max_year)
  } else {
    character(0)
  }
} else {
...
year_choice_names <- if (nrow(plex) > 0) {
  plex %>%
    dplyr::filter(year >= 1950, albumArtist != 'Various Artists') %>%
    dplyr::mutate(choiceLabel = ifelse(
      year > 1999, year, 10 * floor(year / 10)
    )) %>%
    dplyr::distinct(albumArtist, album, choiceLabel) %>%
    dplyr::count(choiceLabel) %>%
    dplyr::arrange(choiceLabel) %>%
    dplyr::mutate(n = stringr::str_c("(", n, ")")) %>%
    tidyr::unite(choiceLabel, n, col = 'choiceLabel', sep = " ") %>%
    dplyr::pull(choiceLabel)
} else {
  character(0)
}

selected_year <- if (nrow(plex) > 0 && any(!is.na(plex$year))) {
  suppressWarnings(max(plex$year, na.rm = TRUE))
} else {
  "All"
}

shiny::radioButtons(inputId = "chooseYear", label = "Select year:",
            choiceValues = c("All", '1950', '1960', '1970', '1980', '1990', year_values), 
...
output$missingMajorWorksTable <- DT::renderDataTable({
  row_limit <- if (is.null(input$missingDiscogsAIN)) 20 else as.numeric(input$missingDiscogsAIN)
  source_filter <- if (is.null(input$missingDiscogsAISource)) "all" else input$missingDiscogsAISource
  min_confidence <- if (is.null(input$missingDiscogsMinConfidence)) 60 else as.numeric(input$missingDiscogsMinConfidence)

  data <- missingDiscogsAICombined() %>%
    dplyr::arrange(.data$rank)

  if (source_filter == "openai") {
    data <- data %>% dplyr::filter(.data$recommendation_source == "openai")
  } else if (source_filter == "heuristic") {
    data <- data %>% dplyr::filter(.data$recommendation_source == "heuristic_fallback")
  }

  data <- data %>%
    dplyr::filter(.data$ai_confidence >= min_confidence) %>%
    dplyr::slice_head(n = row_limit) %>%
    dplyr::mutate(
      recommendation_source = dplyr::if_else(.data$recommendation_source == "openai", "AI agent", "Heuristic fallback"),
      avg_plex_rating_for_artist = round(.data$avg_plex_rating_for_artist, 2),
      baseline_score = round(.data$baseline_score, 2),
      ai_confidence = round(.data$ai_confidence, 0)
    ) %>%
    dplyr::select(
      .data$rank,
...
```{r}
shiny::tags$div(
  id = "missing-plex-discogs-table-wrap",
  style = "height: calc(100vh - 180px);",
  DT::DTOutput("discogsNotInPlexTable", height = "100%")
)
```
...
```{r}
shiny::tags$div(
  id = "missing-plex-lastfm-table-wrap",
  style = "height: calc(100vh - 180px);",
  DT::DTOutput("lastfmNotInPlexTable", height = "100%")
)
```

# Missing from Discogs (AI)

## Controls {.sidebar}

```{r}
shiny::radioButtons(
  inputId = "missingDiscogsAIN",
  label = "Rows",
  choices = c("Top 20" = 20, "Top 50" = 50, "Top 100" = 100),
  selected = 20,
  inline = TRUE
)

shiny::radioButtons(
  inputId = "missingDiscogsAISource",
  label = "Suggestion source",
  choices = c(
    "All" = "all",
    "AI agent only" = "openai",
...
shiny::sliderInput(
  inputId = "missingDiscogsMinConfidence",
...
shiny::actionButton(
  inputId = "refreshMissingDiscogsAI",
  label = "Refresh AI suggestions",
  icon = icon("refresh")
)

shiny::helpText("Runs R/analysis/2026-02-missingMajorWorks.R and reloads this table.")
```

## Row {#missing-discogs-ai-row data-height = "1200"}

### Recommendations

```{r}
shiny::tags$div(
  id = "missing-discogs-ai-table-wrap",
  style = "height: calc(100vh - 180px);",
  DT::DTOutput("missingMajorWorksTable", height = "100%")
)
```
